...
public class Prolog {
    ...
    public void assert_type(String s) {
        term inp=(new prologtokenizer(s)).gettermdot(null);
        if (sv.assert_type(inp) == false) {
            System.out.println("Error assert_typeing: " + s);
        }
    }
	...
	if (args.length == 0) {
	     ...
	     */
	    Prolog p = new Prolog();
	    p.assert_type("father(ken,mark).");
	    p.assert_type("father(ken,ron).");
	    p.assert_type("father(ron,anthony).");
	    p.assert_type("grandfather(X,Z):-father(X,Y),father(Y,Z).");
		...
		Enumeration enum_type = the_answers.keys();
		while (enum_type.hasMoreElements()) {
		    String var = (String)enum_type.nextElement();
...
class program {
    ...
    static void makeprelude() {
        ...
        assert_type(prelude,"member(X,[X|_]).");
        assert_type(prelude,"member(X,[_|H]):-member(X,H).");
        assert_type(prelude,"not(X):-X,!,fail.");
        assert_type(prelude,"not(X).");
        assert_type(prelude,"append([],B,B).");
        assert_type(prelude,"append([A|B],C,[A|D]):-append(B,C,D).");
        assert_type(prelude,"select([X|B],X,B).");
        assert_type(prelude,"select([A|B],X,[A|C]):-select(B,X,C).");
        assert_type(prelude,"reverse(X,XR):-reverse(X,[],XR).");
        assert_type(prelude,"reverse([],XR,XR).");
        assert_type(prelude,"reverse([H|T],TR,XR):-reverse(T,[H|TR],XR).");
        assert_type(prelude,"permutation([],[]).");
        assert_type(prelude,"permutation(LX,[X|LP]):-select(LX,X,L),permutation(L,LP).");
    }
    ...
    static void fillwith(Hashtable to,Hashtable from) {
        Enumeration enum_type=from.elements();
        term oldlist,newlist;
        while(enum_type.hasMoreElements()) {
            oldlist=(term)enum_type.nextElement();
    ...
    public String toString() {
        Enumeration enum_type=user.elements();
        Vector v;
        term t;
        StringBuffer buf=new StringBuffer("\n");
        while(enum_type.hasMoreElements()) {
            term list=(term)enum_type.nextElement();
    ...
    static boolean assert_type(Hashtable h,String s)
    {return assert_type(h,new prologtokenizer(s).gettermdot(null));}
    ...
    static boolean assert_type(Hashtable h,term t) {
    ...
    static boolean assert_typea(Hashtable h,term t) {
        if(t==null)  return false;
        //t=term.skipeq(t);
        term thead=head(t);
        if(thead==null)  return false;
...
class solver {
    ...
    solver(program l) {
        if(ASK==null) {
            ...
            bi("assert_type",1,26);
            bi("assert_typez",1,26);
            bi("assert_typea",1,27);
            bi("retract",1,28);
    ...
    int solve(rack r) {
        ...
        if(bi!=-1) {
            ...
            case 26:
                r.solveoption=r.NOTAGAIN;
                if(assert_type(program.gramconvert(rpred.arg[0].copy())
                          ))
                    return TRUE;
                return ERROR;
            case 27:
                r.solveoption=r.NOTAGAIN;
                if(program.assert_typea(lib.user,
    ...
    boolean assert_type(term X)
    {  return program.assert_type(lib.user,X);
    }
